...
Provides utilitarian methods for general stellar cyber usage.
    ...
    version:    20230530.000    initial 
                ...
                20260318.000    added method STELLAR_UTIL.update_connector 
                20260512.000    added method local_db.update_sync_state mainly to support a retry counter      
"""
...
class STELLAR_UTIL:
    ...
    def get_stellar_case_ai(self, case_id, include=None):
        '''
        case_id:    stellar case identifier as hex
        include:    optional parameter
                    must be one of (or blank which gets all)
                        "details",
                        "summary",
                        "alerts",
                        "comments",
                        ...
                        "aiSummary"
        '''
        path = "/connect/api/v1/ai/cases/detail?id={}".format(case_id)
        if include:
            path += "&include={}".format(include)
        r = self._request_get(path=path)
        r = r.get('data', {})
        if include:
            r = r.get(include, {})
    ...
    def get_license_entities(self, tenant_id='', date='', days_back=0, daily_count=False):
        ret = {}
        return_type = "entity_list"
        if daily_count:
            return_type = "daily_count"
        path = "/connect/api/v1/entity_usages/{}/tenant?".format(return_type)
        has_param = False
        if tenant_id:
        ...
        if date:
            ...
            has_param = True
        # only the daily_count supports the "days" parameter - the entity_list will silently ignore it
        if days_back and return_type == 'daily_count':
            if has_param:
                path += "&"
    ...
    def update_connector(self, connector_name, new_config):
        ret = []
        path = "/connect/api/v1/connectors"
        self.l.info("Updating connector: [{}]".format(connector_name))
        r = self._request_put(path=path, data=new_config)
        ret = r.get('data', [])
        return ret
    ...
    def lookup_update(self, lookup_id, lookup_name, update_list :list, lookup_type=None, tenant_id=None):
        ret = {}
        path = "/connect/api/v1/lookups/{}".format(lookup_id)
        self.l.info("updating lookup group: {}".format(lookup_id))
        if not lookup_type:
            lookup_type="ip"
        if not tenant_id:
...
class local_db():
    ...
    def update_sync_state(self, stellar_case_id, state):
        ts = int(time.time()) * 1000
        sql = 'UPDATE {} SET ts = {}'.format(self.ticket_table_name, ts)
        sql += ', state="{}"'.format(state)
        sql += ' WHERE stellar_case_id = "{}"'.format(stellar_case_id)
        with self.con:
            cur = self.con.cursor()
